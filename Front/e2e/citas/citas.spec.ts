...
async function findExistingStudentAppointment(page: Page) {
  ...
  return {
    id,
    status: (await row.getByText(/Propuesta|Aceptada/i).first().textContent()) ?? '',
  };
}
...
function buildAppointmentValues(
  ...
  const request =
    dashboard.requests.find(
      (item) => item.status === 'ACEPTADA' && PACIENTE_PRUEBA.test(item.patientName),
    ) ?? dashboard.requests.find((item) => item.status === 'ACEPTADA');
  const site = dashboard.practiceSites.find((item) => item.status === 'active');
  const supervisor = dashboard.supervisors.find((item) => item.status === 'active');
  const appointmentType = dashboard.appointmentTypes[0];
  const treatment = dashboard.treatments.find((item) => item.status === 'active');
  const times = slotTimes(slot);

  expect(request, 'Debe existir una solicitud aceptada para agendar citas.').toBeTruthy();
  ...
  expect(appointmentType, 'Debe existir al menos un tipo de cita.').toBeTruthy();
    ...
    async ({ apiBaseUrl, body, method, path }): Promise<ApiResult<T>> => {
      ...
      if (
        ...
        if (refreshResponse.ok && refreshedSession?.accessToken) {
          window.localStorage.setItem(storageKey, JSON.stringify(refreshedSession));
...
async function createAppointmentFromStudentUi(page: Page, slot: number) {
  ...
  await selectDropdownOption(page, 'student-appointment-request', PACIENTE_PRUEBA);
  ...
  const treatment = page.locator('[data-testid^="student-appointment-treatment-option-"]').first();
  ...
  await page.locator('#student-appointment-additional-info').fill(additionalInfo);
  ...
  const appointment = (await createResponse.json()) as StudentAppointment;
  expect(appointment.status).toBe('PROPUESTA');

  await expect(page.locator(`[data-testid="student-appointment-row-${appointment.id}"]`))
...
async function createAcceptedAppointmentViaApi(
  ...
  const studentContext = await browser.newContext({ storageState: SESIONES.estudiante });
  ...
  const patientContext = await browser.newContext({ storageState: SESIONES.paciente });
...
test('E2E-13 | Estudiante propone una cita al paciente', async ({ browser }) => {
...
test('E2E-14 | Paciente acepta la cita propuesta', async ({ browser }) => {
  ...
  const row = page.locator(`[data-testid="patient-appointment-row-${appointmentId}"]`);
  await expect(row).toBeVisible({ timeout: 15_000 });

  if (await row.getByText(/Aceptada/i).first().isVisible({ timeout: 1_500 }).catch(() => false)) {
  ...
  const acceptResponsePromise = page.waitForResponse((response) =>
    response.url().includes(`/patient-portal/appointments/${appointmentId}/status`) &&
  ...
  const appointment = (await acceptResponse.json()) as PatientAppointment;
...
test('E2E-15 | La cita aceptada queda visible para paciente y estudiante', async ({ browser }) => {
  const appointmentId = requireMainAppointmentId();
  ...
  await patientPage.goto(RUTAS.pacienteCitas);
  await patientPage.waitForLoadState('networkidle');
  await expect(patientPage.locator(`[data-testid="patient-appointment-row-${appointmentId}"]`))
  ...
  await studentPage.goto(RUTAS.estudianteCitas);
  await studentPage.waitForLoadState('networkidle');
  await expect(studentPage.locator(`[data-testid="student-appointment-row-${appointmentId}"]`))
    .toContainText(/Aceptada/i, { timeout: 15_000 });
...
test('E2E-16 | Estudiante solicita reprogramacion con mas de 48 horas', async ({ browser }) => {
  const appointmentId = requireMainAppointmentId();
  const context = await browser.newContext({ storageState: SESIONES.estudiante });
  const page = await context.newPage();
  ...
  const row = page.locator(`[data-testid="student-appointment-row-${appointmentId}"]`);
  await expect(row).toContainText(/Aceptada/i, { timeout: 15_000 });
  ...
  const rescheduleResponsePromise = page.waitForResponse((response) =>
    response.url().includes(`/student-portal/appointments/${appointmentId}/reschedule`) &&
    response.request().method() === 'POST',
  );

  await page.getByRole('button', { name: /enviar reprogramacion/i }).click();
  ...
  const appointment = (await rescheduleResponse.json()) as StudentAppointment;
...
test('E2E-18 | Paciente y estudiante pueden cancelar citas aceptadas con mas de 48 horas', async ({ browser }) => {
  ...
  const patientRow = patientPage.locator(`[data-testid="patient-appointment-row-${patientCancelId}"]`);
  await expect(patientRow).toContainText(/Aceptada/i, { timeout: 15_000 });

  const patientCancelResponsePromise = patientPage.waitForResponse((response) =>
    response.url().includes(`/patient-portal/appointments/${patientCancelId}/status`) &&
    response.request().method() === 'PATCH',
  );
  ...
  const patientCancelled = (await patientCancelResponse.json()) as PatientAppointment;
  ...
  const studentRow = studentPage.locator(`[data-testid="student-appointment-row-${studentCancelId}"]`);
  await expect(studentRow).toContainText(/Aceptada/i, { timeout: 15_000 });

  const studentCancelResponsePromise = studentPage.waitForResponse((response) =>
    response.url().includes(`/student-portal/appointments/${studentCancelId}/status`) &&
    response.request().method() === 'PATCH',
  );
  ...
  const studentCancelled = (await studentCancelResponse.json()) as StudentAppointment;
...
test('E2E-19 | Cita futura no se puede finalizar ni valorar', async ({ browser }) => {
  const appointmentId = requireMainAppointmentId();
  const context = await browser.newContext({ storageState: SESIONES.estudiante });
  const page = await context.newPage();
  ...
  const row = page.locator(`[data-testid="student-appointment-row-${appointmentId}"]`);
  await expect(row).toContainText(/Aceptada/i, { timeout: 15_000 });
  await expect(row.getByRole('button', { name: /finalizar|calificar/i })).toHaveCount(0);

  console.log('E2E-19: Cita futura sin acciones de finalizacion o valoracion.');
  await context.close();
});

test('E2E-20 | Valoracion queda reservada para citas finalizadas', async ({ browser }) => {
  const appointmentId = requireMainAppointmentId();
  const context = await browser.newContext({ storageState: SESIONES.paciente });
  const page = await context.newPage();
  ...
  const row = page.locator(`[data-testid="patient-appointment-row-${appointmentId}"]`);
  await expect(row).toContainText(/Aceptada/i, { timeout: 15_000 });
  await expect(row.getByRole('button', { name: /valorar/i })).toHaveCount(0);

  console.log('E2E-20: Una cita aceptada y futura no muestra valoracion hasta finalizar.');
  await context.close();
});
