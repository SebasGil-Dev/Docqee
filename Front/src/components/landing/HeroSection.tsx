...
export function HeroSection({ hero }: HeroSectionProps) {
  const titleStart = hero.titleStart.trim();
  const titleAccent = hero.titleAccent.trim();
  const titleEnd = hero.titleEnd.trim();
  ...
  return (
    <section className="relative overflow-hidden pb-16 pt-4 sm:pb-20 sm:pt-6 lg:min-h-[calc(100svh-5.75rem)] lg:py-[clamp(1rem,2.6svh,2rem)] xl:min-h-[calc(100svh-6.25rem)] xl:py-[clamp(1.35rem,2.8svh,2.4rem)] 2xl:min-h-[calc(100svh-6.75rem)]">
      ...
      <div className="landing-shell mx-auto grid gap-12 px-4 sm:gap-14 sm:px-6 lg:min-h-full lg:grid-cols-[minmax(0,1.02fr)_minmax(22rem,0.92fr)] lg:items-center lg:gap-[clamp(2rem,3.6vw,3.25rem)] lg:px-8 xl:gap-[clamp(2.5rem,4vw,4rem)] xl:px-10 2xl:grid-cols-[minmax(0,1fr)_minmax(34rem,0.92fr)]">
        <div className="space-y-6 lg:pl-[clamp(1.35rem,2.1vw,2.15rem)] lg:space-y-[clamp(1.2rem,2.4svh,1.9rem)] xl:pl-[clamp(1.75rem,2.45vw,2.75rem)] xl:space-y-[clamp(1.4rem,2.6svh,2.2rem)] 2xl:pl-[clamp(2.15rem,2.65vw,3.35rem)]">
          <div className="space-y-5">
            ...
            <h1 className="mx-auto max-w-[20ch] text-center font-headline text-[clamp(1.7rem,8vw,2.55rem)] font-extrabold leading-[1.1] tracking-tight text-ink [text-wrap:balance] sm:max-w-3xl sm:text-5xl sm:leading-[1.06] lg:mx-0 lg:max-w-[13ch] lg:text-left lg:text-[clamp(2.8rem,3.7vw,4rem)] lg:leading-[1.02] xl:max-w-[12ch] xl:text-[clamp(3.15rem,3.9vw,4.5rem)] 2xl:text-[clamp(3.35rem,3.6vw,4.85rem)]">{titleStart} <span className="text-primary">{titleAccent}</span> {titleEnd}</h1>
        ...
        <div className="relative mx-auto -mt-4 w-full max-w-[28rem] sm:mt-0 sm:max-w-[31rem] lg:ml-auto lg:mr-0 lg:max-w-[min(32.5rem,43vw)] xl:max-w-[min(36.5rem,41vw)] 2xl:max-w-[min(40rem,40vw)]">
          <div
            aria-hidden="true"
            className="absolute -right-6 top-8 hidden h-44 w-44 rounded-full bg-glow/45 blur-3xl sm:block xl:h-48 xl:w-48"
          />
          <div className="relative flex justify-center py-2 sm:py-3 xl:py-3.5">
            <div className="relative w-full rounded-[1.9rem] bg-surface-card p-[0.4rem] shadow-float sm:p-[0.5rem] lg:w-fit lg:max-w-full xl:rounded-[2.15rem] xl:p-[0.58rem]">
              <div className="rounded-[1.6rem] bg-surface-low p-[0.4rem] sm:p-[0.5rem] xl:rounded-[1.75rem] xl:p-[0.58rem]">
                <img
                  {...heroImagePriorityProps}
                  alt={hero.image.alt}
                  className="block h-auto w-full rounded-[1.3rem] lg:mx-auto lg:w-auto lg:max-w-full lg:max-h-[min(31.5rem,56svh)] xl:max-h-[min(36rem,63svh)] 2xl:max-h-[min(40rem,67svh)]"
                  decoding="async"
